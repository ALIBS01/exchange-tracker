...
const CryptoList = () => {
  ...
  useEffect(() => {
    const fetchTrending = async () => {
      try {
        ...
        const ids = json.coins.map((coin) => coin.item.id);
        setTrendingIds(ids);
      } catch (err) {
        console.error("Error fetching trending IDs:", err);
      }
    };
  ...
  const { data, loading, error } = useFetchData("/coins/markets", getParams());
  ...
  return (
    <section className="p-4">
      ...
      {loading ? (
        <p className="p-4">Loading cryptocurrencies...</p>
      ) : error ? (
